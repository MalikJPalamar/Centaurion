...
> Personal AI agent cryptographically bound to a single user and maintained over that user's lifetime. Maintains a continuously updated model of the user: developmental stage across six lines (cognitive, emotional, interpersonal, somatic, spiritual, strategic), behavioral patterns, biases, preferences, and biological parameters such as sleep, energy, and somatic state. Functions as a persistent external memory layer that captures and indexes all information the user encounters across sessions. Processes each delegated task through a fixed sequence: load the user model and relevant context, propose an approach, classify the task by novelty, stakes, and reversibility, execute or escalate, record the outcome. Routine low-risk tasks run autonomously; novel, high-stakes, or hard-to-reverse decisions are returned to the user, and the user's ratings of completed work adjust both the user model and the escalation thresholds. Domain-specific workloads — business operations, research, monitoring — run as interchangeable modules on the person-bound core.
