...
> Personal AI agent system designed for use by a single individual over the long term. The agent is linked to one user through cryptographic authentication and is not transferable or shared. It maintains a structured profile of its user, built from an initial psychological and behavioral assessment and updated continuously: the profile records the user's assessed developmental level across six dimensions (cognitive, emotional, interpersonal, somatic, spiritual, strategic), documented preferences and decision patterns, behavioral history from past interactions, and physiological data such as sleep, energy levels, and physical practice. All information the user processes through the system — documents, decisions, conversations, research — is written to a persistent memory database and is retrievable in later sessions. The agent performs tasks on the user's behalf: each task is classified by novelty, stakes, and reversibility; low-risk tasks execute automatically, while high-risk tasks are submitted to the user for approval, and classification thresholds adjust based on the user's numerical ratings of completed work. Domain-specific capabilities, such as business operations, research, and monitoring, are installed as separate task modules. Includes a web dashboard for reviewing agent activity and the stored user profile.
