...
KNOWN_REPO_DEBT = {
    ...
    # PR #90 (PT-7) moved docs/centaurion-wiki and docs/aob-wiki into separate
    # repos and left pointer stubs; these scripts still assert on the old paths.
    "verify-coherence.sh": "asserts on docs/centaurion-wiki/* moved out by PT-7 (PR #90)",
    "verify-knowledge-depth.sh": "asserts on docs/centaurion-wiki/* moved out by PT-7 (PR #90)",
    "verify-memory-integration.sh": "R15.7 asserts on docs/centaurion-wiki/ moved out by PT-7 (PR #90)",
    "verify-multi-runtime.sh": "R18.4 asserts on docs/centaurion-wiki/ moved out by PT-7 (PR #90)",
}
...
PRODUCTION_ONLY = {
    ...
    "verify-hermes-integration.sh": "Requires a live Hermes deploy + ~/browser-harness; set CENTAURION_PRODUCTION=1 to run",
}
